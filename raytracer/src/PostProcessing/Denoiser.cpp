...
Vec3 singlePass(
        ...
        int stepSize) {
    int height = static_cast<int>(color.size());
    if (height == 0) return Vec3();
    int width = static_cast<int>(color[0].size());
    if (width == 0) return Vec3();

    Vec3 outputSum;
    for (int row = 0; row < height; ++row) {
        for (int column = 0; column < width; ++column) {
            const Vec3& centerColor = color[row][column];
            const Vec3& centerNormal = normal[row][column];
            const Vec3& centerPosition = position[row][column];
            ...
            for (int dy = -2; dy <= 2; ++dy) {
                ...
                for (int dx = -2; dx <= 2; ++dx) {
                    ...
                    double spatialW = SPATIAL_FILTER[dy + 2][dx + 2];

                    const Vec3& sampleColor = color[ny][nx];
                    double colorDiff = (sampleColor - centerColor).getLength();
                    double colorW = std::exp(
                        -colorDiff / (settings.colorPhi * settings.colorPhi +
                                      1e-10));

                    const Vec3& sampleNormal = normal[ny][nx];
                    double normalDot = std::max(
                        0.0, centerNormal.dot(sampleNormal));
                    double normalW = normalDot > 0.999
                        ? 1.0
                        : std::pow(normalDot, 128.0 /
                                                  (settings.normalPhi + 1e-10));

                    const Vec3& samplePos = position[ny][nx];
                    double posDiff =
                        (samplePos - centerPosition).getLength();
                    double posW = std::exp(
                        -posDiff / (settings.positionPhi + 1e-10));

                    double weight = spatialW * colorW * normalW * posW;
            ...
            const_cast<Vec3&>(centerColor) = result;
        }
    }
    return outputSum;
}
...
void ATrousDenoiser::denoise(
        ...
        const std::vector<std::vector<Vec3>>& albedoIn,
